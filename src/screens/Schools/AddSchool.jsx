...
const AddSchool = memo(() => {
  const navigation = useNavigation();
  const { top } = useSafeAreaInsets();
  const [createSchool, { isSuccess, isError, error }] = useCreateSchoolMutation();
  const { data: schools, refetch } = useFetchSchoolsQuery();
  ...
  useEffect(() => {
    if (isSuccess) {
      refetch(); 
      navigation.navigate(ROUTES.Schools);
    } else if (isError && error && !showError) {
      // Display the error message
      Alert.alert(
        "Update Error",
        error.data?.message || "Failed to add School."
      );
      setShowError(true);
    }
  ...
  const onSubmit = useCallback(
    (data) => {
        setIsLoading(true);
        createSchool(data).unwrap();
        setIsLoading(false);
        navigation.navigate(ROUTES.Schools);
    },
    [createSchool, setIsLoading]
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.txtCreateAccount}>Add New School</Text>
      ...
      <View style={styles.buttonView}>
        <ButtonPrimary title={"Add School"} onPress={handleSubmit(onSubmit)} />
      </View>
    </View>
  );
});
