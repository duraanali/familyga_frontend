...
const AddDoctor = memo(() => {
  const navigation = useNavigation();
  const { top } = useSafeAreaInsets();
  const [createDoctor, { isSuccess, isError, error }] = useCreateDoctorMutation();
  const { data: doctors, refetch } = useFetchDoctorsQuery();
  const [isLoading, setIsLoading] = useState(false); // New loading state
  const [showError, setShowError] = useState(false);
  const [listHospitalsData, setListHospitalsData] = useState([]);
  const {
    data: hospitals,
  ...
  useEffect(() => {
    if (hospitals && hospitals.hospitals.length > 0) {
      setListHospitalsData(hospitals.hospitals);
    }
  }, [isLoadingHospitals, hospitals]);
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      Alert.alert(
        "Update Error",
        error.data?.message || "Failed to add Doctor."
      );
      setShowError(true);
    }
  ...
  const onSubmit = useCallback(
    (data) => {
        setIsLoading(true);
        createDoctor(data).unwrap();
        setIsLoading(false);
        navigation.navigate(ROUTES.Doctors);
    },
    [createDoctor, setIsLoading]
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.txtCreateAccount}>Add New Doctor</Text>
...
<Controller
   ...
   {listHospitalsData && (
            ...
            render={({ field: { onChange, value } }) => (
              <View style={styles.pickerContainer}>
                ...
                >
                  ...
                  {listHospitalsData &&
                    listHospitalsData?.map((hospital) => (
                      <Picker.Item label={hospital.name} value={hospital.id} />
                    ))}
                </Picker>
              </View>
            )}
      ...
      <View style={styles.buttonView}>
        <ButtonPrimary title={"Add Doctor"} onPress={handleSubmit(onSubmit)} />
      </View>
    </View>
  );
});
