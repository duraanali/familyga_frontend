...
const EditTeacher = memo(({ route }) => {
  const navigation = useNavigation();
  const { top } = useSafeAreaInsets();
  const { item } = route.params;
  const [updateTeacher, { isLoading: isUpdating, error, isSuccess, isError }] = useUpdateTeacherMutation();
  const { data: teachers, refetch } = useFetchTeachersQuery();
  const [isLoading, setIsLoading] = useState(false); // New loading state
  const [showError, setShowError] = useState(false);
  const [listSchoolsData, setListSchoolsData] = useState([]);
  ...
  useEffect(() => {
    if (schools && schools.schools.length > 0) {
      setListSchoolsData(schools.schools);
    }
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      Alert.alert(
        "Update Error",
        error.data?.message || "Failed to add Teachers."
      );
      setShowError(true);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.txtCreateAccount}>Edit Teacher</Text>
